...
exports.createLDPInvoice = createLDPInvoice;
exports.encodeLDPInvoice = encodeLDPInvoice;
exports.decodeLDPInvoice = decodeLDPInvoice;
...
function serializeInvoice(invoice) {
    ...
    let offset = 0;
    buf.writeUInt8(invoice.version, offset);
    offset += 1;
    buf.writeUInt32BE(invoice.timestamp, offset);
    offset += 4;
    buf.writeUInt8(runeIdBuf.length, offset);
    offset += 1;
    runeIdBuf.copy(buf, offset);
    offset += runeIdBuf.length;
    buf.writeBigUInt64BE(invoice.runeAmount, offset);
    offset += 8;
    buf.writeUInt32BE(invoice.lightningAmountSats, offset);
    offset += 4;
    invoice.paymentHash.copy(buf, offset);
    offset += 32;
    invoice.recipientPubkey.copy(buf, offset);
    offset += 32;
    buf.writeUInt32BE(invoice.expiry, offset);
    offset += 4;
    buf.writeUInt8(senderUTXOBuf.length, offset);
    offset += 1;
    if (senderUTXOBuf.length > 0) {
        senderUTXOBuf.copy(buf, offset);
        offset += senderUTXOBuf.length;
    }
 ...
 */
function deserializeInvoice(buf) {
    let offset = 0;
    const version = buf.readUInt8(offset);
    offset += 1;
    const timestamp = buf.readUInt32BE(offset);
    offset += 4;
    const runeIdLen = buf.readUInt8(offset);
    offset += 1;
    const runeId = buf.subarray(offset, offset + runeIdLen).toString('utf8');
    offset += runeIdLen;
    const runeAmount = buf.readBigUInt64BE(offset);
    offset += 8;
    const lightningAmountSats = buf.readUInt32BE(offset);
    offset += 4;
    const paymentHash = Buffer.from(buf.subarray(offset, offset + 32));
    offset += 32;
    const recipientPubkey = Buffer.from(buf.subarray(offset, offset + 32));
    offset += 32;
    const expiry = buf.readUInt32BE(offset);
    offset += 4;
    const senderUTXOLen = buf.readUInt8(offset);
    offset += 1;
 ...
 */
function createLDPInvoice(runeId, runeAmount, recipientPubkey, lightningAmountSats, expiry = 3600, senderUTXO) {
 ...
 */
function encodeLDPInvoice(invoice) {
