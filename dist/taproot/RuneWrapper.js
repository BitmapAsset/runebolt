...
exports.RuneWrapper = void 0;
const uuid_1 = require("uuid");
const logger_1 = require("../utils/logger");
const types_1 = require("../types");
const security_1 = require("../security");
const log = (0, logger_1.createLogger)('RuneWrapper');
class RuneWrapper {
    tapd;
    operations = new Map();
    constructor(tapd) {
     ...
     */
    async wrap(request) {
        if (!security_1.InputValidator.validateAmount(request.amount)) {
            throw new Error('Invalid wrap amount');
        }
        const op = {
            id: (0, uuid_1.v4)(),
            state: types_1.WrapState.PENDING,
     ...
     */
    async confirmLock(opId, lockTxid) {
        const op = this.getOperation(opId);
        if (op.state !== types_1.WrapState.PENDING) {
            throw new Error(`Invalid state for lock confirmation: ${op.state}`);
        }
        if (!security_1.InputValidator.validateTxid(lockTxid)) {
            throw new Error('Invalid lock transaction ID');
        }
        op.lockTxid = lockTxid;
        op.state = types_1.WrapState.RUNE_LOCKED;
    ...
    async unwrap(request) {
        if (!security_1.InputValidator.validateHex(request.assetId, 32)) {
    ...
    getOperation(opId) {
        ...
        if (!op)
            throw new Error(`Wrap operation not found: ${opId}`);
        return op;
    }
    getAllOperations() {
...
exports.RuneWrapper = RuneWrapper;
//# sourceMappingURL=RuneWrapper.js.map